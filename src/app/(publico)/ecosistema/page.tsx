  ...
  return (
    <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-white">
      <div className="h-1 w-40 overflow-hidden rounded-full bg-slate-100">
        <div className="h-full w-1/3 animate-loaderPulse rounded-full bg-gradient-to-r from-fuchsia-500 to-cyan-500" />
      </div>
...
export default function EcosistemaPage() {
  ...
  const seleccionadoNombre = useMemo(
    () => modulos.find((m) => m.id === selectedId)?.nombre ?? 'ninguno',
    [selectedId]
  );

  return (
    <main className="relative w-full flex-1 overflow-hidden bg-white">
      {/* Sutil aura de color de fondo para que el blanco no sea plano */}
      <div
        className="pointer-events-none absolute inset-0 z-0"
        style={{
          background:
            'radial-gradient(1200px 700px at 50% 42%, rgba(124,92,224,0.06), transparent 60%), radial-gradient(900px 600px at 78% 80%, rgba(18,165,201,0.05), transparent 60%)',
        }}
      />
